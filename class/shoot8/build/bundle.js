  ...
  */
    ...
    */
		...
		value: function render() {
			this.game.debug.text("distance:" + this.enemy.enemyBody.x - this.player.playerBody.x, 20, 20);
...
var Enemy = function (_Phaser$Sprite) {
	...
	function Enemy(_game, _x, _y, _player, _ground) {
		...
		_this.groupEnemy = _this.game.add.group();
  ...
  */
  ...
  /**/
		_this.enemyBody.body.syncBounds = true;
		_this.enemyBody.body.allowGravity = false;
		_this.enemyBody.body.immovable = true;
		_this.groupEnemy.add(_this.enemyBody);
		...
		_this.groupEnemy.add(_this.enemyBullet);
	...
	_createClass(Enemy, [{
		...
		value: function move() {
			...
			if (_customConfig2.default.flag.checkStop) {
				//this.setEnemyTreeX();
				this.setEnemyLaunchSpeed(this.player.playerBody.x, this.groupEnemy.x);
